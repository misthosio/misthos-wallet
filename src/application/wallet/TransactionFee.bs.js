...
function estimateInputWeight(nCoSigners, nPubKeys) {
  return (((306 + Caml_int32.imul(nCoSigners, 73) | 0) + 1 | 0) + Caml_int32.imul(nPubKeys, 34) | 0) + 3 | 0;
}
...
function inputCost(nCoSigners, nPubKeys, fee) {
  var weight = estimateInputWeight(nCoSigners, nPubKeys);
  return fee.times(weight / 4);
}

function canPayForItself(fee, input) {
  var weight = estimateInputWeight(input[/* nCoSigners */4], input[/* nPubKeys */5]);
...
function estimate(outputs, inputs, fee, network) {
  var weight = (baseWeight + List.fold_left((function (t, o) {
          ...
          return t + estimateInputWeight(i[/* nCoSigners */4], i[/* nPubKeys */5]) | 0;
        }), 0, inputs) | 0;
  return fee.times(weight / 4);
}
