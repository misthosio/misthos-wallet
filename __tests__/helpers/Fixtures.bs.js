...
function loadFixture(fileName) {
  try {
    return decodeFixture(Json.parseOrRaise(Fs.readFileSync(fileName + "-fixture.json", "utf8")));
  }
  catch (exn){
    return undefined;
  }
...
function withCached($staropt$star, $staropt$star$1, scope, description, sessionsGenerator, generator, testBody) {
  var load = $staropt$star !== undefined ? $staropt$star : true;
  var persist = $staropt$star$1 !== undefined ? $staropt$star$1 : true;
  var replacedName = description.replace((/ /g), "_");
  var cacheFileName = basePath + (scope + ("-" + replacedName));
  var match = loadFixture(cacheFileName);
  var match$1;
  var exit = 0;
  if (load && match !== undefined) {
    var match$2 = match;
    match$1 = /* tuple */[
      match$2[0],
